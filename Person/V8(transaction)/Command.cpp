...
void _charge_Konto_BargeldCmd::execute()
{
	_charge_Konto_Bargeld = new charge_Konto_Bargeld();
	_charge_Konto_Bargeld->execute(login_stmt->activated_pers_id);
}

void _Nachschlage_KontostandCmd::execute()
{
	_Nachschlage_Kontostand = new Nachschlage_Kontostand();
	_Nachschlage_Kontostand->execute(login_stmt->activated_pers_id);
}
...
void RemoteControllerTest::execute()
	{
		...
		while (while_i)
		{
			...
			if (_login_stmt->bool_login_stmt == true)
			{
				...
				//cout << "\nAktuelle Kontostand :" + obj_bring_Kontostand.execute(_login_stmt->activated_pers_id);
			}
			...
			cout <<
				"\n==============================================================================="
				"\n0.Program Ausschalten"  
				"\n1.Einloggen   2.Ausloggen   3.Ueberweisen Geld   4.Sehen Kontostand"
			...
			case 2:
				...
				break;
			case 3:
				controller.setCmd(cmd_Charge_Konto_Bargeld);
				controller.pressButton();
				break;
			case 4:
				controller.setCmd(cmd_Nachschlage_Kontostand);
				controller.pressButton();
				break;
			case 10:
