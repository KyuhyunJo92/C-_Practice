...
public:
	...
	void execute();
	char* sql_purchase_transaction(int, int);
	//constructor
...
class Nachschlage_Kontostand
{
public:
	char* execute(int _pers_id);
};

class bring_Kontostand
{
public:
	int execute(int);
};

class charge_Konto_Bargeld
{
public:
	char* execute(int _pers_id);
};
...
{
public:
	double execute(int _prod_id);
};

class show_haltemenge
{
public:
	int execute(int);
};
